...
public class DynatraceDeleteAnomalieDetectionActionEngine implements ActionEngine {
	...
	public SampleResult execute(Context context, List<ActionParameter> list) {
		...
		final String dynatraceApiKey = parsedArgs.get(DynatraceDeleteAnomalieDetectionOption.DynatraceApiKey.getName())
				.get();
		final Optional<String> dynatraceManagedHostname = parsedArgs
				.get(DynatraceDeleteAnomalieDetectionOption.DynatraceManagedHostname.getName());
		final Optional<String> proxyName = parsedArgs
				.get(DynatraceDeleteAnomalieDetectionOption.NeoLoadProxy.getName());
		final Optional<String> optionalTraceMode = parsedArgs
				.get(DynatraceDeleteAnomalieDetectionOption.TraceMode.getName());
		...
		try {
			...
			if (listofids == null)
				return ResultFactory.newErrorResult(context, STATUS_CODE_BAD_CONTEXT,
						"There were no Anamalies created during this test");
